...
class AdminSearchService
  ...
  def search_purchases(query: nil, product_title_query: nil, purchase_status: nil, creator_email: nil, license_key: nil, transaction_date: nil, last_4: nil, card_type: nil, price: nil, expiry_date: nil, order_id: nil, limit: nil)
    purchases = Purchase.order(created_at: :desc)

    if order_id.present?
      purchase = purchases.find_by_external_id_numeric(order_id.to_i)
      return Purchase.none unless purchase
      purchases = purchases.where(id: purchase.id)
    end
